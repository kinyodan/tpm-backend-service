  ...
  def rpc_get_books
    ...
    books_data.each do |data|
      ...
      if found
        ...
        items = [url: data['url'], name: data['name'], isbn: data['isbn'], authors: data['authors'], numberOfPages: data['numberOfPages'], publisher: data['publisher'], country: data['country'], mediaType: data['mediaType'], releases: data['releases']]
